...
$sql = "SELECT count(*)
		...
		WHERE i.finalizado_sos=".$finalizado_v." AND v.id=i.via AND r.id=i.referencia AND ta.id=i.tipo_atencion AND u.id=i.id_usuario $where";
...
$sql = "SELECT i.id,i.periodo,i.codigo,i.guardado_sos,i.finalizado_sos,i.guardado_adm_vial,i.finalizado_adm_vial,
		...
		FROM ".$_SESSION[APL]->bd->nombre_bd[0].".dvm_incidente as i,
			...
			".$_SESSION[APL]->bd->nombre_bd[0].".dvm_tipo_atencion as ta,
			".$_SESSION[APL]->bd->nombre_bd[0].".dvm_usuarios as u
		WHERE i.finalizado_sos=".$finalizado_v." AND v.id=i.via AND r.id=i.referencia AND ta.id=i.tipo_atencion AND u.id=i.id_usuario
			$where
		ORDER BY $order
		LIMIT $start , $limit";
$rs = $_SESSION[APL]->bd->getRs($sql);
